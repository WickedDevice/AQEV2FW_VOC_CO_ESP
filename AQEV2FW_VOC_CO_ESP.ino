...
void restore(char * arg) {
    ...
    if (strncmp(arg, "defaults", 8) == 0) {
        ...
        configInject(F("sampling 5, 600, 60\r")); // sample every 5 seconds, average over 1 minutes, report every minute
        ...
        eeprom_write_block("aqev2_voc_co_esp", (void *) EEPROM_UPDATE_FILENAME, 32);
    }
...
boolean publishHeartbeat() {
    ...
    snprintf(scratch, SCRATCH_BUFFER_SIZE-1,
             ...
             "\"converted-value\":%d,"
             "\"firmware-version\":\"%s\","
             "\"publishes\":[\"voc\",\"co\",\"temperature\",\"humidity\"%s],"
             "\"counter\":%lu"
